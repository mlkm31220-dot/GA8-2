...
def process_select_phase(body: Dict[str, Any]) -> JSONResponse:
    ...
    if (
        not isinstance(forbidden_features, list)
        or not all(isinstance(x, str) for x in forbidden_features)
    ):
        valid_structure = False
    ...
    if (
        "INVALID_INPUT" not in reason_codes
        and retained_rows
    ):
        ...
        for r in retained_rows[1:]:
            ...
            candidate_feats = candidate_feats.intersection(
                set(r["features"].keys())
            )
        ...
        for feat in candidate_feats:
            ...
            for r in retained_rows:
                ...
                if (
                    not isinstance(feat_obj, dict)
                    or "availableAt" not in feat_obj
                ):
                    feat_eligible = False
                    break
    ...
    if isinstance(trials, list):
        ...
        for t in trials:
            ...
            if (
                not is_non_negative_safe_int(t_id)
                or t_id in seen_trial_ids
...
def process_evaluate_phase(body: Dict[str, Any]) -> JSONResponse:
    ...
    if (
        ...
        or not is_non_negative_safe_int(selected_trial_id)
        or not isinstance(dataset_digest, str)
    ):
        reason_codes.add("INVALID_INPUT")

    if (
        not isinstance(metric_floor, (int, float))
        or isinstance(metric_floor, bool)
        or not (0.0 <= metric_floor <= 1.0)
        or not math.isfinite(metric_floor)
    ):
        reason_codes.add("INVALID_INPUT")
    ...
    if (
        not is_non_negative_safe_int(bytes_processed)
        or not is_non_negative_safe_int(max_bytes)
    ):
        reason_codes.add("INVALID_INPUT")
    ...
    if (
        stored_run is None
        or stored_run.get("selectedTrialId") is None
    ):
        lineage_valid = False

    elif (
        stored_run.get("selectedTrialId") != selected_trial_id
        or stored_run.get("datasetDigest") != dataset_digest
    ):
        lineage_valid = False
    ...
    if (
        is_non_negative_safe_int(bytes_processed)
        and is_non_negative_safe_int(max_bytes)
    ):
    ...
    if (
        ...
        and "INVALID_INPUT" not in reason_codes
    ):
        ...
        if isinstance(req_slices, dict):
            ...
            for s_name, s_floor in req_slices.items():
                ...
                if s_name not in slice_map:
                    ...
                    s_acc = round_12(
                        s_correct / len(s_rows)
                    )

                    if s_acc < s_floor:

                        reason_codes.add(
                            f"SLICE_FLOOR:{s_name}"
                        )
    ...
    return JSONResponse(
        ...
        content={
            ...
            "bytesProcessed": (
                bytes_processed
                if is_non_negative_safe_int(bytes_processed)
                else None
            ),
            "reasonCodes": sorted_reasons
        }
...
async def handle_promote(request: Request):
    ...
    champion_version_raw = body.get(
        "championVersion"
    )

    champion_version_in = (
        str(champion_version_raw)
        if champion_version_raw is not None
        else ""
    )
    ...
    if (
        ...
        or not isinstance(versions_list, list)
    ):
    ...
    if (
        not is_finite_number(accuracy_floor)
        or not (0.0 <= accuracy_floor <= 1.0)
    ):
        valid_policy = False
    ...
    if (
        not is_finite_number(max_latency_ms)
        or max_latency_ms < 0
    ):
        valid_policy = False
    ...
    # ========================================================
    # FIX 1:
    # Stored alias is authoritative once it exists.
    # Otherwise use championVersion from the request.
    # ========================================================

    current_champion_version = ALIAS_STORE.get(
        "champion",
        champion_version_in
    )
    ...
    for v_item in versions_list:
        ...
        if (
            not v_id
            or not is_canonical_version_str(v_id)
        ):
        ...
        if (
            eval_obj is None
            or not isinstance(eval_obj, dict)
        ):
            ...
            e_artifact = eval_obj.get(
                "artifactDigest"
            )

            e_dataset = eval_obj.get(
                "datasetDigest"
            )

            e_schema = eval_obj.get(
                "schemaDigest"
            )

            if (
                not isinstance(e_artifact, str)
                or e_artifact != artifact_digest
            ):
                v_codes.add(
                    "ARTIFACT_MISMATCH"
                )
            ...
            if (
                not isinstance(e_dataset, str)
                or e_dataset != dataset_digest
            ):
                v_codes.add(
                    "DATASET_MISMATCH"
                )
            ...
            if (
                not isinstance(e_schema, str)
                or e_schema != schema_digest
            ):
                v_codes.add(
                    "SCHEMA_MISMATCH"
                )
            ...
            if (
                is_finite_number(acc)
                and 0.0 <= acc <= 1.0
            ):

                if round_12(acc) < round_12(
                    accuracy_floor
                ):
                ...
                for (
                    ...
                    if req_name not in slices_obj:
                        ...
                        if not is_finite_number(
                            ...
                            v_codes.add(
                                f"SLICE_FLOOR:{req_name}"
                            )

        sorted_v_codes = sorted(
            list(v_codes),
            key=lambda x: x.encode("utf-8")
        )
    ...
    eligible_versions = [
        str(v["version"])
        for v in valid_version_objects
    ]

    eligible_versions.sort(
        key=lambda x: int(x)
    )
    ...
    ranked_eligible = sorted(
        valid_version_objects,
        key=rank_key
    )

    best_challenger = ranked_eligible[0]

    best_challenger_version = str(
    ...
    # --------------------------------------------------------
    # No valid champion found
    # --------------------------------------------------------

    if champion_item is None:

        ALIAS_STORE["champion"] = (
    ...
    champion_acc = round_12(
        champion_item["evaluation"]["accuracy"]
    )

    challenger_acc = round_12(
        best_challenger["evaluation"]["accuracy"]
    )

    acc_diff = round_12(
        challenger_acc - champion_acc
    )
    ...
    # 0 > 0 = False
    #
    # Therefore retain champion.
    # ========================================================

    if (
        best_challenger_version
        != current_champion_version
        and acc_diff > round_12(
            min_improvement
        )
    ):
